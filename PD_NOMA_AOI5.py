...
def load_and_combine_episode_data(run_dir, M_total, num_slots, num_episodes):
    """
    Load and combine telemetry data from episode-wise .npy files into a single merged data structure.
    Each episode data is loaded using the filename format:
    "slotwise_dataEp{ep}U{M_total}S{num_slots}.npy"
    """
    combined_data = defaultdict(list)

    # Loop over episodes and load data
    for ep in range(1, num_episodes + 1):
        filename = f"slotwise_dataEp{ep}U{M_total}S{num_slots}.npy"
        data = load_episode_telemetry(run_dir, filename)

        # Merge the episode data into combined data
        for uid, rows in data.items():
            combined_data[uid].extend(rows)
...
def merge_chunks_efficiently(run_dir, M_total, num_slots, num_episodes, final_filename=None):
    """
    Merge all episode chunks into one final .npy file, ensuring correct episode-wise filenames.
    The filenames should be in the format: slotwise_dataEp{ep}U{M_total}S{num_slots}.npy
    """
    ...
    # Correct filename pattern to load chunk files based on episode numbers
    chunk_files = sorted(glob.glob(os.path.join(chunks_dir, f"slotwise_dataEp*_U{M_total}S{num_slots}.npy")))

    for chunk_file in chunk_files:
        try:
            # Load each episode data chunk one by one using the correct episode-based filename
            chunk_data = np.load(chunk_file, allow_pickle=True).item()
            for k, v in chunk_data.items():
                merged[k].extend(v)
        except Exception as e:
            print(f"[FINAL] Failed to read chunk {chunk_file}: {e}")

    # Save the merged data to the final .npy file
...
for ep in range(1, num_episodes + 1):
    ...
    telemetry.save_episode_npy(RUN_DIR, filename=f"slotwise_dataEp{ep}U{M_total}S{num_slots}.npy")
    telemetry.save_slotwise_npy(RUN_DIR, filename=f"slotwise_dataUEp{ep}{M_total}S{num_slots}.npy")
...
data = load_and_combine_episode_data(RUN_DIR, M_total, num_slots, num_episodes)
