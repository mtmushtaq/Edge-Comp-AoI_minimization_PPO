...
D_min              = 8.0
D_sum              = D_max + D_min
tau                = 10 #round(D_sum/ 2) #np.sqrt(2.0),2)  # near/far threshold by your spec
battery_max = 1
AoI_max = 400
...
def make_run_dir(M_total, num_slots):
    ...
    name = f"AoI_U{M_total}_S{num_slots}_EP{num_episodes}_{gamma_th_db}RNDM"
...
def assign_slots_for_frame_random(
    ep, frame,
    pairs, singles, num_slots,
    build_pair_state,
    system_succ_hist,
    rng=None
):
    """
    Pure random slot assignment (no policy), with SAR state logging:
      - Randomize item order and slot order (no low-index bias)
      - Build a state per item (pair uses (uN,uF); single uses (u,u))
      - Log SAR: (ep, frame, slot_idx, state, action=slot_idx)
      - Return assigned_slots and slot_map consistent with your decoder loop
    """
    rng = rng or np.random.default_rng()

    # 0) Pool items and ensure enough to fill all slots
    items = [("pair", p) for p in pairs] + [("single", s) for s in singles]
    if len(items) < num_slots:
        raise RuntimeError(
            f"Not enough items to fill {num_slots} slots (got {len(items)}). "
            f"Ensure pairs + singles cover capacity."
        )

    # 1) Shuffle items and slots (removes index bias and fallback bias)
    rng.shuffle(items)
    slots = np.arange(num_slots, dtype=int)
    rng.shuffle(slots)

    assigned_slots = []
    slot_map = {}

    # 2) Assign the first num_slots items to the shuffled slots
    for (typ, obj), slot_idx in zip(items[:num_slots], slots):
        # Build the state exactly like in PPO runs (so SAR plots work the same)
        if typ == "pair":
            uN, uF = obj
        ...
        # 3) Log SAR decision BEFORE reward is known
        if 'sar_logger' in globals() and sar_logger is not None:
            sar_logger.log(ep, frame, int(slot_idx), s_np.squeeze().tolist(), int(slot_idx))

        # 4) Record assignment for the simulator/decoder
        if typ == "pair":
            assigned_slots.append(("pair", (uN, uF), int(slot_idx)))
            slot_map[int(slot_idx)] = (uN, uF)
        else:
            assigned_slots.append(("single", u, int(slot_idx)))
            slot_map[int(slot_idx)] = u

    # 5) Invariants (exactly one item per slot, all slots present)
    if len(slot_map) != num_slots:
        missing = [s for s in range(num_slots) if s not in slot_map]
        raise RuntimeError(
            f"Expected {num_slots} slots, but only {len(slot_map)} assigned. Missing: {missing}"
        )
    assert set(slot_map.keys()) == set(range(num_slots)), "slot_map must have all slot keys"
...
for ep in range(1, num_episodes + 1):
    ...
    for frame in range(frames_per_episode):
        ...
        assigned_slots, slot_map = assign_slots_for_frame_random(
            ep=ep, frame=frame,
