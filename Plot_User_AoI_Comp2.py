...
def plot_mean_episode_end_avg_vs_users_multi(
    ...
    Uses IEEE single-column figure styling.
    """
...
Base_TH = f"AOI_Threshold_Users"
Base_Greedy = f"AoI_Greedy_Users"
Base_PPO = f"AoI_PPO_Users"
Base_RNDM = f"AOI_Users_RNDM"
run_dirs_PPO    = [os.path.join(Base_PPO, f"AoI_U{u}_S{num_slots}_EP{num_episodes}_RewardNS") for u in user_counts]
run_dirs_RNDM   = [os.path.join(Base_RNDM, f"AoI_U{u}_S{num_slots}_EP{num_episodes}_RNDM" ) for u in user_counts]
run_dirs_Greedy = [os.path.join(Base_Greedy, f"AoI_U{u}_S{num_slots}_EP{num_episodes}_GREEDY") for u in user_counts]
run_dirs_Threshold = [os.path.join(Base_TH, f"AoI_U{u}_S{num_slots}_EP{num_episodes}_policyV2") for u in user_counts]

plot_mean_episode_end_avg_vs_users_multi(
    run_dirs_PPO, run_dirs_RNDM, run_dirs_Greedy, run_dirs_Threshold,
    user_counts, num_slots, frames_per_episode, num_episodes,
    out_pdf="AoI_vs_Users_PPO_RNDM_Greedy_Threshold_IT.png"
)
