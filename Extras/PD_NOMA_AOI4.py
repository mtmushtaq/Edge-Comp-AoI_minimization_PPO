...
class Telemetry:
    ...
    def __init__(self, run_dir, M_total, num_slots, num_episodes):
        ...
        Parameters:
            run_dir (str): The directory where the data will be stored.
            M_total (int): Total number of users.
        ...
        # Create a temporary directory to hold episode-wise memory files
        self.mm_dir = os.path.join(run_dir, "mm")
        os.makedirs(self.mm_dir, exist_ok=True)

        # Memory-mapped arrays for storing data for each episode
        self.AAOI_ep_mm = np.memmap(
            os.path.join(self.mm_dir, f"AAOI_ep_U{M_total}S{num_slots}.dat"),
            dtype=np.float32, mode="w+", shape=(num_episodes,)  # one entry for each episode
        )
    ...
    def flush_episode_to_temp_file(self, ep, M_total, num_slots):
        ...
        # Save data to memory-mapped arrays
    ...
    def finalize_run(self, run_dir, M_total, num_slots, final_filename="slotwise_dataU{M_total}S{num_slots}.npy",
                     keep_chunks=True):
        """
        Merge all episode chunks into one final .npy file.
        """
        ...
        chunks_dir = os.path.join(run_dir, "chunks")
        chunk_files = sorted(glob.glob(os.path.join(chunks_dir, f"slotwise_data_ep*_U{M_total}S{num_slots}.npy")))
...
telemetry.finalize_run(RUN_DIR, M_total, num_slots, final_filename=f"slotwise_dataU{M_total}S{num_slots}.npy", keep_chunks=True)
